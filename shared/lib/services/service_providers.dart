...
const _chatBaseUrl =
    String.fromEnvironment('CHAT_BASE_URL', defaultValue: 'http://10.0.2.2:8081');
...
final callServiceProvider = Provider<CallService>((ref) {
  final hmsCallService = HMSCallService(); // Use HMSCallService
  ref.onDispose(() {
    hmsCallService.dispose();
  });
  return hmsCallService;
});

// Provider for MeetingService
final meetingServiceProvider = Provider<MeetingService>((ref) {
  final mockMeetingService = MockMeetingService();
  ref.onDispose(() {
    mockMeetingService.dispose();
  });
